  ...
  execute = async(user_id: string, data: SchemaTypeZod["SchemaCreateSystemService"]): Promise<void> => {
    try {
      ...
      await prisma.$transaction(async(tx) => {
        ...
        const { id, user_id, event, phone, email_alert } = ShedulingSystem;
        ...
        const payloadOutboxPattern = ensureJsonObject(resultPayload.data, { jobId: id, phone, email_alert, userId: user_id });
